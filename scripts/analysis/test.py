...
def analyse_lecture(db, cursor, crowdcount, seats_occupied):
    ...
    for id, data, _ in probable_seats:
        ...
        for ts, occupency_id, sc, mc in data:
            ...
            seat = edit_id(id)
            cursor.execute(
                "INSERT OR REPLACE INTO clean_seats (occupency_id, seat, engagement) VALUES (?, ?, ?)",
                (occupency_id, seat, engagement),
            )
        ...
        for i in df.index:
            ...
            if timestamp > int(next_lecture_ts):
                if curr_lecture:
                    ...
                    crowdcount = (
                        int(sum(crowdcounts[q1:q3]) / (q3 - q1)) if q3 > q1 else 0
                    )
                    print(
                        "Analyzing lecture",
                        curr_lecture,
                    ...
                    analyse_lecture(db, cursor, crowdcount, seats_occupied)
                    seats_occupied = defaultdict(list)
                    crowdcounts = []
            ...
            if curr_lecture:
                ...
                for seat in result:
                    seats_occupied[seat[2]].append(
                        (timestamp, seat[1], seat[3], seat[4])
                    )

...
def get_lecture_co2_data():
    ...
    data = ([{"sensor_id": row["sensor_id"], "timestamp": row["timestamp"], "co2": row["co2"]} for row in co2_data])
    return data
...
def get_xy_sensor_coordinates():
    ...
    data = ([{"sensor_id": row["sensor_id"], "x": row["x"], "y": row["y"]} for row in coordinates])
    return data
...
def plot_avg_lecture_co2_per_sensor():
    ...
    sensor_xy = [wgb_to_seat_coords(sensor["x"], sensor["y"]) for sensor in sensor_coords]
    ...
    scatter = ax.scatter(sensor_x, sensor_y, c=sensor_co2, cmap=cmap, norm=norm, s=100, edgecolors='black', label="Sensors")
...
# Run `python -m scripts.vis.lecture_co2`
